...
describe("Login Router", () => {
  ...
  test("login returns 401 for an invalid username", async () => {
    const response = await request(app)
      .post("/api/login")
  ...
  test("login returns 401 for an invalid password", async () => {
    const response = await request(app)
      .post("/api/login")
      .send({ username: "mockUser", password: "invalid_password" })
      .expect(401)
      .expect("Content-Type", /application\/json/);
  ...
  test("login returns 400 for missing username or password", async () => {
